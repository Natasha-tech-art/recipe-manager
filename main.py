...
# Import meal planner frame view sheet 
...
class MainApplication(ctk.CTk):
    ...
    def show_dashboard(self):
        ...
        # --- PERMANENT PERSISTENT LAYOUT DESIGN ---
        # 1. Image Gallery stays fixed at the top of the view workspace
        self.build_image_gallery_grid()

        # 2. Scroll view displays directly underneath the gallery, expanding to fill remaining space
    ...
    def build_image_gallery_grid(self):
        """Creates the food gallery inside a fixed container area at the top"""
        self.gallery_frame = ctk.CTkFrame(self.recipe_list_frame, fg_color="transparent")
        self.gallery_frame.pack(fill="x", padx=10, pady=(5, 10))
        ...
        food_items = [
            ...
            {"title": "snack", "path": "assets/snack.jpg", "row": 0, "col": 3},
            {"title": "dessert", "path": "assets/dessert.jpg", "row": 0, "col": 4}
        ]
        ...
        for item in food_items:
            try:
                ...
                # Made size slightly more compact so all 5 look clean side-by-side in a single row
                ctk_img = ctk.CTkImage(light_image=raw_image, dark_image=raw_image, size=(130, 90))
                self.keep_alive_images.append(ctk_img)

                cell = ctk.CTkFrame(grid_container, fg_color="transparent")
                cell.grid(row=item["row"], column=item["col"], padx=12, pady=5)

                img_lbl = ctk.CTkLabel(cell, image=ctk_img, text="")
                img_lbl.pack()

                txt_lbl = ctk.CTkLabel(cell, text=item["title"], font=("Helvetica", 11, "bold"), text_color="#4CAF50")
                txt_lbl.pack(pady=(2, 0))
            except Exception:
                error_box = ctk.CTkLabel(grid_container, text=f"[Missing {item['title']}]", font=("Helvetica", 10), text_color="gray50")
                error_box.grid(row=item["row"], column=item["col"], padx=12, pady=5)
    ...
    def display_correct_view(self, recipes_list):
        """Clears out and populates the lower scroll workspace list dynamically"""
    ...
    def populate_scroll_list(self, recipes):
        if not recipes:
            no_rec_lbl = ctk.CTkLabel(self.recipe_scroll, text="No matching recipes found in this slot.", font=("Helvetica", 13, "italic"), text_color="gray60")
            no_rec_lbl.pack(pady=20)
            return

        for res in recipes:
            cuisine_tag = res.get('cuisine', 'General')
            cat_tag = res.get('category', 'Unassigned')
            btn_text = f"🍽️  {res['title']} ({cuisine_tag}) — Type: {cat_tag}"
    ...
    def view_recipe_details(self, recipe):
        ...
        meta_text = f"Cuisine: {recipe.get('cuisine', 'General')}  |  Category Slot: {recipe.get('category', 'Unassigned')}"
        ctk.CTkLabel(scroll_content, text=meta_text, font=("Helvetica", 13, "italic"), text_color="gray").pack(pady=(0, 20), anchor="w")
    ...
    def open_edit_recipe(self, recipe):
        ...
        self.edit_cat_cmb.set(recipe.get('category', 'Breakfast'))
